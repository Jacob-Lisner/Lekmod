...
void CvLuaGame::RegisterMembers(lua_State* L)
{
	...
	Method(CheckLekmodUpdate);
	// Also expose as a global so FrontEnd/MainMenu can call it even when Game methods are awkward.
	lua_pushcfunction(L, lCheckLekmodUpdate);
	lua_setglobal(L, "LekmodCheckUpdate");
#endif
...
namespace
{
	struct LekmodVersionUrl
	{
		const wchar_t* host;
		const wchar_t* path;
	};

	static const LekmodVersionUrl LEKMOD_VERSION_URLS[] =
	{
		{ L"raw.githubusercontent.com", L"/EnormousApplePie/Lekmod/main/LekmodInstaller/github_setup/versions.json" },
		{ L"cdn.jsdelivr.net", L"/gh/EnormousApplePie/Lekmod@main/LekmodInstaller/github_setup/versions.json" },
	};

	bool LekmodHttpGetHttps(const wchar_t* host, const wchar_t* path, std::string& outBody, DWORD& outError)
	{
		outBody.clear();
		outError = 0;

		HINTERNET hSession = WinHttpOpen(
			L"LekmodVersionCheck/1.0",
		...
		// GitHub requires modern TLS; Civ5's default stack often fails without this.
		// Hardcoded for older Windows SDK headers (VS2013).
...
#define WINHTTP_FLAG_SECURE_PROTOCOL_TLS1_2 0x00000800
		...
		for (;;)
		{
			...
			DWORD dwRead = 0;
			if (!WinHttpReadData(hRequest, &buffer[0], dwAvailable, &dwRead) || dwRead == 0)
			{
				outError = GetLastError();
				break;
			}
			outBody.append(&buffer[0], &buffer[0] + dwRead);
	...
	bool LekmodGetVersionCachePath(char* outPath, size_t outPathSize)
	{
		if (!outPath || outPathSize < 32)
		{
			return false;
		}
		...
		{
			sprintf_s(documents, "%s\\Documents", userProfile);
		}
		else
		{
			return false;
		...
		sprintf_s(outPath, outPathSize, "%s\\My Games\\Sid Meier's Civilization 5\\Lekmod\\version_cache.txt", documents);
		return true;
	}

	bool LekmodReadVersionCache(std::string& outVersion, std::string& outFileId)
	{
		outVersion.clear();
		outFileId.clear();

		char path[MAX_PATH] = {0};
		if (!LekmodGetVersionCachePath(path, MAX_PATH))
		{
			return false;
		}
		...
		// VS2008 toolset (v90): no C++11 lambdas / auto
		for (int pass = 0; pass < 2; ++pass)
		{
			char* s = (pass == 0) ? line1 : line2;
			size_t n = strlen(s);
			while (n > 0 && (s[n - 1] == '\n' || s[n - 1] == '\r' || s[n - 1] == ' ' || s[n - 1] == '\t'))
			{
				s[--n] = '\0';
	...
	void LekmodWriteVersionCache(const std::string& version, const std::string& fileId)
	{
		char path[MAX_PATH] = {0};
		if (!LekmodGetVersionCachePath(path, MAX_PATH))
		{
		...
		// Ensure directory exists: ...\My Games\Sid Meier's Civilization 5\Lekmod
		char dir[MAX_PATH] = {0};
		strcpy_s(dir, path);
		...
		{
			*slash = '\0';
			CreateDirectoryA(dir, NULL); // Lekmod folder; parent usually exists
			// Also ensure Civ5 folder exists under My Games (best-effort)
		...
		fprintf(fp, "%s\n%s\n", version.c_str(), fileId.c_str());
		fclose(fp);
	}

	bool LekmodHttpGetVersionsJson(std::string& outBody, DWORD& outError)
	{
		outBody.clear();
		outError = 0;
		for (size_t i = 0; i < sizeof(LEKMOD_VERSION_URLS) / sizeof(LEKMOD_VERSION_URLS[0]); ++i)
		{
			DWORD err = 0;
			if (LekmodHttpGetHttps(LEKMOD_VERSION_URLS[i].host, LEKMOD_VERSION_URLS[i].path, outBody, err))
			{
				outError = 0;
				return true;
			}
	...
	void LekmodNormalizeVersion(const char* szIn, std::string& outVersion)
	{
		outVersion.clear();
		if (!szIn)
		{
		...
		while (*p)
		{
			if ((*p == 'v' || *p == 'V') && isdigit(static_cast<unsigned char>(*(p + 1))))
			{
				outVersion.push_back('v');
				++p;
				while (*p && (isdigit(static_cast<unsigned char>(*p)) || *p == '.'))
				{
					outVersion.push_back(*p);
					++p;
				}
				return;
			}
	...
	bool LekmodParseVersionParts(const std::string& version, std::vector<int>& parts)
	{
		parts.clear();
		size_t i = 0;
		...
		while (i < version.size())
		{
			...
			int value = 0;
			while (i < version.size() && isdigit(static_cast<unsigned char>(version[i])))
			{
	...
	int LekmodCompareVersions(const std::string& a, const std::string& b)
	{
		std::vector<int> pa;
		std::vector<int> pb;
		if (!LekmodParseVersionParts(a, pa) || !LekmodParseVersionParts(b, pb))
		{
			return 0;
		}
	...
	bool LekmodFindLatestFromVersionsJson(const std::string& json, std::string& outVersion, std::string& outFileId)
	{
		outVersion.clear();
		outFileId.clear();
		...
		while (pos < json.size())
		{
			const size_t keyPos = json.find("\"v", pos);
			if (keyPos == std::string::npos)
			{
				break;
			}

			size_t verEnd = keyPos + 2;
			while (verEnd < json.size() && (isdigit(static_cast<unsigned char>(json[verEnd])) || json[verEnd] == '.'))
			{
				++verEnd;
			}
			if (verEnd >= json.size() || json[verEnd] != '\"')
			{
				pos = keyPos + 2;
				continue;
			}

			// Require this to look like a JSON key: "vX.Y":
			size_t colon = verEnd + 1;
			...
			std::string version = json.substr(keyPos + 1, verEnd - (keyPos + 1)); // includes leading v

			// Search for file_id within this version object (until next "vN key or end)
			size_t nextKey = json.find("\"v", verEnd + 1);
			size_t searchEnd = (nextKey == std::string::npos) ? json.size() : nextKey;
			const size_t fileIdKey = json.find("\"file_id\"", colon);
			std::string fileId;
			if (fileIdKey != std::string::npos && fileIdKey < searchEnd)
			{
				size_t q1 = json.find('\"', fileIdKey + 9);
				if (q1 != std::string::npos && q1 < searchEnd)
				{
					size_t q2 = json.find('\"', q1 + 1);
					if (q2 != std::string::npos && q2 < searchEnd)
					{
						fileId = json.substr(q1 + 1, q2 - q1 - 1);
					}
				}
			}

			if (outVersion.empty() || LekmodCompareVersions(version, outVersion) > 0)
			{
				outVersion = version;
				outFileId = fileId;
			}
...
// Game.CheckLekmodUpdate(localVersion) / LekmodCheckUpdate(localVersion)
// -> { Success, Latest, FileId, IsNewer, DownloadUrl, Error, Source }
int CvLuaGame::lCheckLekmodUpdate(lua_State* L)
{
	const char* szLocalVersion = luaL_optstring(L, 1, "");
	std::string localVersion;
	LekmodNormalizeVersion(szLocalVersion, localVersion);
	...
	std::string body;
	const bool bFetched = LekmodHttpGetVersionsJson(body, fetchError);
	if (bFetched && LekmodFindLatestFromVersionsJson(body, latest, fileId))
	{
		source = "http";
		LekmodWriteVersionCache(latest, fileId);
	}
	else
	{
		// Fallback: installer-written cache in Documents\My Games\...\Lekmod\version_cache.txt
		if (LekmodReadVersionCache(latest, fileId))
		{
			source = "cache";
		}
		else
		{
			...
			const int t = lua_gettop(L);
			lua_pushboolean(L, 0);
			lua_setfield(L, t, "Success");
			lua_pushnumber(L, static_cast<lua_Number>(fetchError));
			lua_setfield(L, t, "Error");
			lua_pushstring(L, "http-and-cache-miss");
			lua_setfield(L, t, "Source");
			return 1;
		}
	}

	const bool bIsNewer = (!localVersion.empty() && LekmodCompareVersions(latest, localVersion) > 0);

	lua_createtable(L, 0, 7);
	const int t = lua_gettop(L);
	lua_pushboolean(L, 1);
	lua_setfield(L, t, "Success");
	lua_pushstring(L, latest.c_str());
	lua_setfield(L, t, "Latest");
	lua_pushstring(L, fileId.c_str());
	lua_setfield(L, t, "FileId");
	lua_pushboolean(L, bIsNewer ? 1 : 0);
	lua_setfield(L, t, "IsNewer");
	lua_pushstring(L, source.c_str());
	lua_setfield(L, t, "Source");

	if (!fileId.empty())
	{
		std::string url = "https://drive.google.com/uc?export=download&id=";
		url += fileId;
		lua_pushstring(L, url.c_str());
		lua_setfield(L, t, "DownloadUrl");
	}
